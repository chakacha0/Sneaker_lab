...
export async function fetchPriceRange() {
  const response = await fetch("http://localhost:8000/products/price-range");
  return response.json();
}

export async function fetchAvailableSizes() {
  const response = await fetch("http://localhost:8000/products/available-sizes");
  return response.json();
}
...
export async function fetchProductSizes(productId) {
  const res = await fetch(`http://localhost:8000/products/${productId}/sizes`);
  return res.json();
}

export async function addProductStock(productId, size, quantity) {
  const response = await fetch(`http://localhost:8000/products/${productId}/stock`, {
    method: "POST",
    headers: {
...
export async function checkProductStock(productId) {
  const response = await fetch(`http://localhost:8000/products/${productId}/has-stock`);
...
export async function searchProducts(query) {
  const response = await fetch(`http://localhost:8000/products/search?q=${encodeURIComponent(query)}`);
  if (!response.ok) {
